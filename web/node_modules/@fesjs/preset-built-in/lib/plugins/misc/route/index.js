...
const logger = new _compiler.Logger('fes:router'); //   pages
//  ├── index.vue         # 根路由页面 路径 /
//  ├── *.vue             # 模糊匹配 路径 *
//  ├── a.vue             # 路径 /a
//  ├── b
...
const isProcessFile = function (path) {
  ...
  return (0, _fs.statSync)(path).isFile() && ['.vue', '.jsx', '.tsx'].includes(ext);
};

const isProcessDirectory = function (path, item) {
  const component = (0, _path.join)(path, item);
  return (0, _fs.statSync)(component).isDirectory() && !['components'].includes(item);
};

const checkHasLayout = function (path) {
  const dirList = (0, _fs.readdirSync)(path);
  return dirList.some(item => {
    if (!isProcessFile((0, _path.join)(path, item))) {
      return false;
    }

    const ext = (0, _path.extname)(item);
    const fileName = (0, _path.basename)(item, ext);
    return fileName === 'layout';
  });
};

const getRouteName = function (parentRoutePath, fileName) {
  const routeName = _path.posix.join(parentRoutePath, fileName);
...
const getComponentPath = function (parentRoutePath, fileName, config) {
  ...
  return _path.posix.join(`@/${pagesName}/`, parentRoutePath, fileName);
};
...
const getRoutePath = function (parentRoutePath, fileName) {
  ...
  return _path.posix.join(parentRoutePath, fileName);
};

function getRouteMeta(content) {
  const ast = _utils.parser.parse(content, {
    sourceType: 'module',
  ...
  if (defineRouteExpression) {
    const argument = (0, _utils.generator)(defineRouteExpression.expression.arguments[0]);
...
let cacheGenRoutes = {}; // TODO 约定 layout 目录作为布局文件夹，
...
const genRoutes = function (parentRoutes, path, parentRoutePath, config) {
  ...
  dirList.forEach(item => {
    ...
    if (isProcessFile(component)) {
      const ext = (0, _path.extname)(item);
      const fileName = (0, _path.basename)(item, ext); // 路由的path
      ...
      cacheGenRoutes[routePath] = true; // 路由名称
      ...
      let content = (0, _fs.readFileSync)(component, 'utf-8');
      let routeMeta = {};

      if (ext === '.vue') {
        const {
          descriptor
        } = (0, _compilerSfc.parse)(content);
        const routeMetaBlock = descriptor.customBlocks.find(b => b.type === 'config');
        routeMeta = routeMetaBlock !== null && routeMetaBlock !== void 0 && routeMetaBlock.content ? JSON.parse(routeMetaBlock.content) : {};
  ...
  dirList.forEach(item => {
    if (isProcessDirectory(path, item)) {
      ...
      const nextParentRouteUrl = _path.posix.join(parentRoutePath, item);
...
const getRoutesJSON = function ({
  ...
  return JSON.stringify(clonedRoutes, replacer, 2).replace(/"component": ("(.+?)")/g, (global, m1, m2) => `"component": ${m2.replace(/\^/g, '"')}`).replace(/\\r\\n/g, '\r\n').replace(/\\n/g, '\r\n');
};
...
function _default(api) {
  ...
  const absCoreFilePath = (0, _path.join)(namespace, 'routes.js');
  ...
  api.onGenerateFiles(async () => {
    const routesTpl = (0, _fs.readFileSync)((0, _path.join)(__dirname, 'template/routes.tpl'), 'utf-8');
    ...
    api.writeTmpFile({
      ...
      content: (0, _fs.readFileSync)((0, _path.join)(__dirname, 'template/runtime.tpl'), 'utf-8')
    });
  });
  api.addCoreExports(() => [{
